    ...
    def update(self, tilemap, movement=(0, 0)):
        ...
        for rect in tilemap.physics_rects_around(self.pos):
            if entity_rect.colliderect(rect):
                if frame_movement[0] > 0: # whenever the player moves to the right
                    entity_rect.right = rect.left
                    self.collisions['right'] = True
                if frame_movement[0] < 0: # whenever the player moves to the left
        ...
        for rect in tilemap.physics_rects_around(self.pos):
            if entity_rect.colliderect(rect):
                if frame_movement[1] > 0: # whenever the player moves to positive y
                    entity_rect.bottom = rect.top
                    self.collisions['down'] = True
                if frame_movement[1] < 0: # whenever the player moves to negetive y
...
class Player(PhysicsEntity):
    ...
    def update(self, tilemap, movement=(0, 0)):
        ...
        if abs(self.dashing) > 50: # dash cooldown
